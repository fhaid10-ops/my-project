...
0595243553`,
  ...
  financing: {
    ...
    loanTermMonths: 60,
    /** خيارات المدة التي يختار منها العميل (بالسنوات) — من الأطول للأقصر */
    loanTermOptionsYears: [5, 4, 3, 2, 1],
...
في حال لديك اي استفسارات، انا بخدمتك.`,
    /** قائمة أرقام من اللوحة — متابعة تقديم، مو حملة ترحيب */
...
في حال تم التقديم ارسل رقم الطلب`,
  ...
  /**
   * إعدادات الإرسال الصادر من اللوحة (متابعة جماعية)
   * أرقام محافظة لتقليل خطر حظر واتساب عند الإرسال الجماعي.
   */
  outbound: {
    /** التأخير الافتراضي بين كل رسالة */
    delayMs: 10000,
    /** أقل تأخير مسموح به من الواجهة/الـ API */
    minDelayMs: 8000,
    /** أقصى دفعة لموجة بلس/القوائم — متابعة بدون متابعة ترسل للكل مرة واحدة */
    maxBatchSize: 250,
    /**
...
0595243553`,
    ...
    portalAccountLocked: `حسابك في البورتال مقفل حالياً.

تواصل مع دعم الشركة السعودية للتمويل لفتح الحساب، وبعدها تقدر تسجّل من نفس رابط التقديم وتكمل الطلب.

لمتابعة الطلب تواصل مع
عبدالرحمن
0595243553`,
