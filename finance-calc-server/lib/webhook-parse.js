...
function extractChoiceFromMultiline(text) {
  ...
  if (
    /^(مدني|متقاعد|عسكري|[1-4]|لا يوجد|مدعوم|غير مدعوم|قديم|نعم|لا|civilian|retired|military|تمويل شخصي|شراء مديونية|مبالغ التمويل|إيقاف خدمات|ايقاف خدمات|حلول تمويلية|ساعات الدوام|موقعنا|رقم الموظفين|رقم المساعد|menu_[1-8])$/i.test(
      last
    )
  ) {
    return last;
...
function extractTrailingMenuChoice(text) {
  ...
  const re =
    /(تمويل شخصي|شراء مديونية|مبالغ التمويل|إيقاف خدمات|ايقاف خدمات|حلول تمويلية|ساعات الدوام|موقعنا|رقم الموظفين|رقم المساعد)$/i;
  const m = t.match(re);
  if (!m) return t;
  const choice = m[1];
